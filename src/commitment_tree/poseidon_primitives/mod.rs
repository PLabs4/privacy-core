//! The Poseidon algebraic hash function — generic core only.
//!
//! Vendored from `halo2_poseidon` (the crate `halo2_gadgets` re-exports). The
//! Pallas-specific spec (`P128Pow5T3`), field tables (`fp`/`fq`), `test_vectors`, and
//! all `#[cfg(test)]` blocks were dropped — they pulled `pasta_curves` and aren't used
//! by the BN254 Merkle spec, which generates its own constants via `generate_constants`.
//! No `pasta_curves` / `halo2_proofs` / `halo2_gadgets` dependency.
